...
void listaMedicoMenu(NodeMed **lista){
    ...
    while(opc != 0){
            ...
            switch(opc)
            {
                ...
                case 2: removerMedico(&medico,lista);break;  
                case 3: break;
...
void removerMedico(Medico *medico,NodeMed **lista){
	...
	char nome[maxnome];
    int encontrou = 0;
    printf("Digite o nome do medico a remover: ");
    fflush(stdin);
    gets(nome);

    while (!encontrou && aux != NULL){
		if (!strcmp(aux->medico.nomeMedico, nome)){ 
			encontrou = 1;
	}
		else{
			ant = aux;
