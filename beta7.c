...
void renameBackup(int type){
    ...
    if(type == 0){
        remove("medicos.txt");
        result = rename( "medicos_backup.txt", "medicos.txt" );
    }else{
        remove("utentes.txt");
        result = rename( "utentes_backup.txt", "utentes.txt" );
    }
    if( result != 0 )
      printf( "[ERRO] Falha ao carregar o BACKUP");
    else
      printf( "[AVISO]Backup carregado com SUCESSO");
...
void ficheirosMenu(NodeMed **listaMed,NodeUtente **listaUtente){
    ...
    while(opc != 0){
                ...
                switch(opc)
                {
                    case 1: guardarDadosFicheiro(*listaMed,*listaUtente,1,0);break;
                    case 2: guardarDadosFicheiro(*listaMed,*listaUtente,0,0);break;
...
void editarDadosUtente(NodeUtente **lista,NodeMed *listaMed){
    ...
    while(aux != NULL && encontrou == 0){
        if(aux->utente.codigoUtente == codigo){
            ...
            system("cls");
            printf("--------------------------------------------------------\n");
            printf("| Alterando os dados do Utente %s (%li) \t\t\t|\n",aux->utente.nomeUtente,aux->utente.codigoUtente);
            printf("--------------------------------------------------------");
            printf("\n|[1..] Nome                                            |");
            printf("\n|[2..] Numero                                          |");
            printf("\n|[3..] Medico                                          |");
